...
PageTopNavigationSubHeaderID = 34
EntityID = 10
%>
...
if (SearchListingID<>"")then
	set Command1 = Server.CreateObject("ADODB.Command")
		with command1	
			.ActiveConnection = connect
			.CommandText = "Castles_System_Listings_SearchbyID_ForBroker"
			.Parameters.Append .CreateParameter("@RETURN_VALUE", 3, 4)
...
if ((SearchListingID="")AND(SearchedAddress<>""))then
	set Command1 = Server.CreateObject("ADODB.Command")
		with command1	
			.ActiveConnection = connect
			.CommandText = "Castles_System_Listings_SearchbyAddress_ForBroker"
			.Parameters.Append .CreateParameter("@RETURN_VALUE", 3, 4)
			.Parameters.Append .CreateParameter("@Page", 200, 1,200,Page)
			.Parameters.Append .CreateParameter("@RecsPerPage", 200, 1,200,RecsPerPage)
			.Parameters.Append .CreateParameter("@BrokerID", 200, 1,200,EntityPrimaryKeyValue)
...
if ((SearchListingID="")and(SearchedAddress="")and(SearchedStates<>"")and(SearchedSizes<>"")) then
	...
	if (NOT StatusNoFilter AND NOT SizesNoFilter AND NOT StatesNoFilter) then	
		SQLStmt = SQLStmt & " Castles_Listings.Deleted <> 'Y' AND Castles_Listings.BrokerID = " & EntityPrimaryKeyValue & " AND Castles_Listings.StatusID = '" & SearchedApartmentStatus & "' AND Castles_Listings.StateProvinceID ="	
		for each z in SizesArray 
			for each x in StatesArray
				if counter = 1 then
					SQLStmt = SQLStmt & Trim(x) 
				else
					SQLStmt = SQLStmt & " OR Castles_Listings.Deleted <> 'Y' AND Castles_Listings.BrokerID = " & EntityPrimaryKeyValue & " AND Castles_Listings.StateProvinceID = " & Trim(x) & " AND Castles_Listings.StatusID = " & SearchedApartmentStatus	
				end if				
	...
	if (NOT StatusNoFilter AND NOT SizesNoFilter AND StatesNoFilter) then	
		SQLStmt = SQLStmt & " Castles_Listings.Deleted <> 'Y' AND Castles_Listings.BrokerID = " & EntityPrimaryKeyValue & " AND Castles_Listings.StatusID = '" & SearchedApartmentStatus & "' AND Castles_Listings.PropertyTypeID ="	
		for each z in SizesArray 
			if counter = 1 then
				SQLStmt = SQLStmt & Trim(z) 
			else
				SQLStmt = SQLStmt & " OR Castles_Listings.Deleted <> 'Y' AND Castles_Listings.BrokerID = " & EntityPrimaryKeyValue & " AND Castles_Listings.StatusID = " & SearchedApartmentStatus 	
			end if				
	...
	if (StatusNoFilter AND NOT SizesNoFilter AND StatesNoFilter) then	
		SQLStmt = SQLStmt & " Castles_Listings.Deleted <> 'Y' AND Castles_Listings.BrokerID = " & EntityPrimaryKeyValue & " AND Castles_Listings.PropertyTypeID ="	
		for each z in SizesArray 
			if counter = 1 then
				SQLStmt = SQLStmt &  Trim(z)
			else
				SQLStmt = SQLStmt & " OR Castles_Listings.Deleted <> 'Y' AND Castles_Listings.BrokerID = " & EntityPrimaryKeyValue & " AND Castles_Listings.PropertyTypeID = " & Trim(z) 	
			end if				
			SQLStmt = SQLStmt & " AND Castles_Listings.ListPrice >= " & PriceFrom & " AND Castles_Listings.ListPrice <= " & PriceTo
			counter = counter + 1
		Next
	...
	if (NOT StatusNoFilter AND SizesNoFilter AND StatesNoFilter) then	
		SQLStmt = SQLStmt & " Castles_Listings.Deleted <> 'Y' AND Castles_Listings.BrokerID = " & EntityPrimaryKeyValue & " AND Castles_Listings.StatusID = " & SearchedApartmentStatus 						
		SQLStmt = SQLStmt & " AND Castles_Listings.ListPrice >= " & PriceFrom & " AND Castles_Listings.ListPrice <= " & PriceTo
	end if
	
	if (StatusNoFilter AND SizesNoFilter AND StatesNoFilter) then	
		SQLStmt = SQLStmt & " Castles_Listings.Deleted <> 'Y' AND Castles_Listings.BrokerID = " & EntityPrimaryKeyValue					
		SQLStmt = SQLStmt & " AND Castles_Listings.ListPrice >= " & PriceFrom & " AND Castles_Listings.ListPrice <= " & PriceTo
	end if
	...
	if (NOT StatusNoFilter AND SizesNoFilter AND NOT StatesNoFilter) then	
		...
		for each x in StatesArray
			if counter = 1 then
				SQLStmt = SQLStmt & Trim(x) & " AND Castles_Listings.StatusID = " & SearchedApartmentStatus
			else
				SQLStmt = SQLStmt & " OR Castles_Listings.Deleted <> 'Y' AND Castles_Listings.BrokerID = " & EntityPrimaryKeyValue & " AND Castles_Listings.StateProvinceID = " & Trim(x) & " AND Castles_Listings.StatusID = " & SearchedApartmentStatus 	
			end if				
			SQLStmt = SQLStmt & " AND Castles_Listings.ListPrice >= " & PriceFrom & " AND Castles_Listings.ListPrice <= " & PriceTo
	...
	if (StatusNoFilter AND NOT SizesNoFilter AND NOT StatesNoFilter) then
		SQLStmt = SQLStmt & " Castles_Listings.Deleted <> 'Y' AND Castles_Listings.BrokerID = " & EntityPrimaryKeyValue & " AND Castles_Listings.StateProvinceID = "	
		for each z in SizesArray 
			for each x in StatesArray
				if counter = 1 then
					SQLStmt = SQLStmt & Trim(x) 
				else
					SQLStmt = SQLStmt & " OR Castles_Listings.Deleted <> 'Y' AND Castles_Listings.BrokerID = " & EntityPrimaryKeyValue & " AND Castles_Listings.StateProvinceID = " & Trim(x)	
				end if				
				SQLStmt = SQLStmt & " AND Castles_Listings.PropertyTypeID = " & Trim(z) & " AND Castles_Listings.ListPrice >= " & PriceFrom & " AND Castles_Listings.ListPrice <= " & PriceTo
				counter = counter + 1
			Next
...
with command1
	.ActiveConnection = connect
	.CommandText = "Castles_System_StateProvinceAndCountry_ABV_MyListings_DynamicList"
	.Parameters.Append .CreateParameter("@RETURN_VALUE", 3, 4)
	.Parameters.Append .CreateParameter("@BrokerID",200,1,200,EntityPrimaryKeyValue)
	.CommandType = 4
	.CommandTimeout = 0
...
set Command1 = Server.CreateObject("ADODB.Command")
	with command1	
		.ActiveConnection = connect
		.CommandText = "Castles_System_PropertyTypes_MyListings_DynamicList"
		.Parameters.Append .CreateParameter("@RETURN_VALUE", 3, 4)
		.Parameters.Append .CreateParameter("@BrokerID",200,1,200,EntityPrimaryKeyValue)
		.CommandType = 4
		.CommandTimeout = 0
