...
PageTopNavigationSubHeaderID = 46
EntityID = 13
%>
...
BrokerID = EntityPrimaryKeyValue
'Generates Appropriate Stored Proc. Call & Creates SearchResults Array 
Function DCSystemSimpleSearchHeader(BrokerID,Page,RecsPerPage,SearchOnEntity,SearchColumn,SearchValue)
	DQ = """"
	SearchColumn = CleanForDrive(SearchColumn)
	SearchValue = CleanForDrive(SearchValue)
	...
	If SearchColumn <> "" Then
		If Len(SearchValue) <= 3 Then
			...
			With Command1	
				.ActiveConnection = Connect
				.CommandText = "Castles_System_" & SearchOnEntity & "_System_Search_3Char"
				.Parameters.Append .CreateParameter("@RETURN_VALUE", 3, 4)
		...
		Else
			...
			With Command1	
				.ActiveConnection = Connect
				.CommandText = "Castles_System_" & SearchOnEntity & "_System_Search"
				.Parameters.Append .CreateParameter("@RETURN_VALUE", 3, 4)
				.Parameters.Append .CreateParameter("@BrokerID", 200, 1,200,BrokerID)
				.Parameters.Append .CreateParameter("@Page", 200, 1,200,Page)
				.Parameters.Append .CreateParameter("@RecsPerPage", 200, 1,200,RecsPerPage)
		...
		If Not SearchResults.EOF Then
			...
			DCSystemSimpleSearchHeader = SearchResultsArray
		Else
			DCSystemSimpleSearchHeader = null
		End If
	End if
End Function
...
if Len(SearchColumn) = 0 then
	SearchColumn = "Castles_BrokerMessages.ListingID"
end if

If Len(SearchColumn) <> 0 Then
	SearchResultsReturned = DCSystemSimpleSearchHeader(BrokerID,Page,RecsPerPage,SearchOnEntity,SearchColumn,SearchValue)
	If Len(SearchValue) <> 0 Then
        ...
        <tr> 
          <td> 
            <table width="750" border="0" cellspacing="0" cellpadding="3">
              ...
              <form name="SearchRecords" method="post" onSubmit="return Search();" action="searchmessages.asp">
                <tr> 
...
%>
        ...
        <form name="DeleteRecords" method="post" onSubmit="return DeleteCheckedRecords();" action="drivemessages.asp?DCDataDriverType=SQLMultiDelete&SearchColumn=<%=SearchColumn%>&SearchValue=<%=SearchValue%>&Page=<%=Page%>&TotalRecords=<%=TotalRecords%>">
          <tr> 
            <td> 
              <table width="100%" border="0" cellspacing="0" cellpadding="0" height="20">
                ...
                <tr height="20"> 
                  ...
                  <td bgcolor="<%=TitleBar%>" width="20%" class="CastlesTextHeader"><b>Date/Time</b></td>
                  <td bgcolor="<%=TitleBar%>" width="1%">&nbsp;</td>
                  <td bgcolor="<%=TitleBar%>" width="10%" class="CastlesTextHeader"><b>ListingID</b></td>
				  <td bgcolor="<%=TitleBar%>" width="1%">&nbsp;</td>
                  <td bgcolor="<%=TitleBar%>" width="15%" class="CastlesTextHeader"><b>User Name</b></td>
                  <td bgcolor="<%=TitleBar%>" width="1%">&nbsp;</td>
                  <td bgcolor="<%=TitleBar%>" width="10%" class="CastlesTextHeader"><b>Phone</b></td>
                  <td bgcolor="<%=TitleBar%>" width="1%">&nbsp;</td>
                  <td bgcolor="<%=TitleBar%>" width="20%" class="CastlesTextHeader"><b>Email</b></td>
                  <td bgcolor="<%=TitleBar%>" width="1%">&nbsp;</td>
                  <td bgcolor="<%=TitleBar%>" width="5%" class="CastlesTextHeader"><b>Actions</b></td>
                  <td bgcolor="<%=TitleBar%>" width="1%">&nbsp;</td>
...
if Len(SearchColumn) <> 0 then
	if Not IsNull(SearchResultsReturned) then
		...
		Field_ID = 0
		Field_BrokerMessageID = 1
		Field_ListingID = 2
		Field_MessageDateTime = 3
		Field_UserName = 4
		Field_TelNumber = 5
		Field_EmailAddress = 6
		Field_MoreRecords = 7

		For SearchResultsArrayRowCounter = 0 to SearchResultsArrayNumRows
			BrokerMessageID = SearchResultsReturned(Field_BrokerMessageID,SearchResultsArrayRowCounter)
			ListingID = SearchResultsReturned(Field_ListingID,SearchResultsArrayRowCounter)
			MessageDateTime = SearchResultsReturned(Field_MessageDateTime,SearchResultsArrayRowCounter)
			UserName = SearchResultsReturned(Field_UserName,SearchResultsArrayRowCounter)
...
%>
                ...
                <tr bgcolor="<%=BgColor%>"> 
                  <td width="1%">&nbsp;</td>
                  <td width="20%" class="CastlesTextBody"><%=MessageDateTime%></td>
                  <td width="1%" align="center">&nbsp;</td>
                  <td width="10%" class="CastlesTextBody"><%=ListingID%></td>
                  <td width="1%" align="center">&nbsp;</td>
                  <td width="15%" class="CastlesTextBody"><%=UserName%></td>
                  <td width="1%" align="center">&nbsp;</td>
...
%>
                    <a href="viewMessage.asp?BrokerMessageID=<%=BrokerMessageID%>&SearchColumn=<%=Server.URLEncode(SearchColumn)%>&SearchValue=<%=Server.URLEncode(SearchValue)%>&Page=<%=Page%>&TotalRecords=<%=TotalRecords%>" class="normal"><b><%=WordPhrase_View%></b></a>&nbsp;&nbsp;&nbsp;&nbsp; 
                    <%
End If
If inStr(EntityModificationAccessString,"(2)" ) <> 0 Then
%>
                    <a href="viewMessage.asp?BrokerMessageID=<%=BrokerMessageID%>&SearchColumn=<%=Server.URLEncode(SearchColumn)%>&SearchValue=<%=Server.URLEncode(SearchValue)%>&Page=<%=Page%>&TotalRecords=<%=TotalRecords%>" class="normal"><b><%=WordPhrase_Edit%></b></a>&nbsp;&nbsp;&nbsp;&nbsp; 
                    <%
End If
If inStr(EntityModificationAccessString,"(6)" ) <> 0 Then
FullName = FirstName & " " & LastName
%>
                    <a href="javascript:onClick=PopNotes('1','<%=BrokerMessageID%>','<%=Server.URLEncode(FullName)%>');" class="normal"><b><%=WordPhrase_Notes%></b></a> 
                    <%
End If
%>
                  </td>
...
If inStr(EntityModificationAccessString,"(3)" ) <> 0 Then
%>
                    <input type="checkbox" name="DeleteRecordID<%=DeleteCount%>" value="<%=BrokerMessageID%>">
                    <%
End If
%>
                    &nbsp;&nbsp;</td>
