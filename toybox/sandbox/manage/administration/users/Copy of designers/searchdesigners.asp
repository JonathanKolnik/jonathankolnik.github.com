...
PageTopNavigationSubHeaderID = 12
EntityID = 5
%>
...
TranslateCount = TranslateCount + 1
WordPhrase_Designer = TranslationResultsArray(Field_TranslatedWordPhrase,TranslateCount)
TranslateCount = TranslateCount + 1
...
function PopNotes(EntityID,EntityPrimaryKeyValue,PersonalName){
	EntityName = "<%=WordPhrase_Designer%>"
        ...
        <tr> 
          <td> 
            <table width="750" border="0" cellspacing="0" cellpadding="3">
              ...
              <form name="SearchRecords" method="post" onSubmit="return Search();" action="searchdesigners.asp">
                <tr> 
...
%>
        ...
        <form name="DeleteRecords" method="post" onSubmit="return DeleteCheckedRecords();" action="drivedesigners.asp?DCDataDriverType=SQLMultiDelete&SearchColumn=<%=SearchColumn%>&SearchValue=<%=SearchValue%>&Page=<%=Page%>&TotalRecords=<%=TotalRecords%>">
          <tr> 
          <td> 
            <table width="100%" border="0" cellspacing="0" cellpadding="0" height="20">
              ...
              <tr height="20"> 
                ...
                <td bgcolor="<%=TitleBar%>" width="1%">&nbsp;</td>
                <td bgcolor="<%=TitleBar%>" width="10%" class="CastlesTextHeader"><b><%=WordPhrase_CompanyName%></b></td>
                <td bgcolor="<%=TitleBar%>" width="1%">&nbsp;</td>
                ...
                <td bgcolor="<%=TitleBar%>" width="1%">&nbsp;</td>
                <td bgcolor="<%=TitleBar%>" width="15%" class="CastlesTextHeader"><b><%=WordPhrase_EmailAddress%></b></td>
                <td bgcolor="<%=TitleBar%>" width="1%">&nbsp;</td>
...
if Len(SearchColumn) <> 0 then
	if Not IsNull(SearchResultsReturned) then
		...
		Field_DirectLine = 6
		Field_MobileTelNumber = 7
		Field_EmailAddress = 8
		Field_Active = 9
		Field_MoreRecords = 10
...
%>
                ...
                <tr bgcolor="<%=BgColor%>"> 
                  ...
                  <td width="10%" class="CastlesTextBody"><%=CompanyName%></td>
                  <td width="1%" align="center">&nbsp;</td>
...
%>
                    <a href="viewdesigner.asp?DesignerID=<%=DesignerID%>&SearchColumn=<%=Server.URLEncode(SearchColumn)%>&SearchValue=<%=Server.URLEncode(SearchValue)%>&Page=<%=Page%>&TotalRecords=<%=TotalRecords%>" class="normal"><b><%=WordPhrase_View%></b></a>&nbsp;&nbsp;&nbsp;&nbsp; 
                    <%
End If
If inStr(EntityModificationAccessString,"(2)" ) <> 0 Then
%>
                    <a href="editdesigner.asp?DesignerID=<%=DesignerID%>&SearchColumn=<%=Server.URLEncode(SearchColumn)%>&SearchValue=<%=Server.URLEncode(SearchValue)%>&Page=<%=Page%>&TotalRecords=<%=TotalRecords%>" class="normal"><b><%=WordPhrase_Edit%></b></a>&nbsp;&nbsp;&nbsp;&nbsp; 
                    <%
End If
If inStr(EntityModificationAccessString,"(6)" ) <> 0 Then
FullName = FirstName & " " & LastName
%>
                    <a href="javascript:onClick=PopNotes('1','<%=DesignerID%>','<%=Server.URLEncode(FullName)%>');" class="normal"><b><%=WordPhrase_Notes%></b></a> 
                    <%
End If
%>
                  </td>
...
If inStr(EntityModificationAccessString,"(3)" ) <> 0 Then
%>
                    <input type="checkbox" name="DeleteRecordID<%=DeleteCount%>" value="<%=DesignerID%>">
<%
End If
%>
                    &nbsp;&nbsp;</td>
