...
'Syncronizes Cookies Self Update Of Designer
...
Select Case DCDataDriverType
	Case "SQLInsert"
		...
		'Inserts Designer Information
		DBObjectDestination = "Castles_Designers"
		FormType = "Request.Form"
		FileServerDestination = ""
		FileFate = ""
		EmailDestination = ""
		DataUniqueKey = "DesignerID"
		DataParentNode = ""
		DataExceptions = "<!DesignerID!><!EntityLanguageID!><!EntitySystemLoginID!><!HeaderCount!><!SubHeaderCount!><!NickName!><!UserName!><!Password!><!ShowHelpContent!><!CreateNotes!><!ViewNotes!><!DeleteNotes!><!Submit!>" & DataExceptionsString
		DataCookies = ""
		DataSessions = ""
		DataExtraFields = ""
		Cnekt = Connect
		
		DesignerID = DCDataDriver(DCDataDriverType,DBObjectDestination,FileServerDestination,FileFate,EmailDestination,DataUniqueKey,DataParentNode,DataCookies,DataSessions,DataExtraFields,DataExceptions,Cnekt,EntityID,FormType)
		
		'Tracks Modification of Designer Account
		EntityID = 5
		EntityModificationTypeID = 2
		EntityModificationLogger SystemLoginID,EntityID,DesignerID,EntityModificationTypeID,IPAddress 

		'Creates System Login Username & Password
		SystemLoginTypeID = 3
		EntityID = 5
		Deleted = "N"
		EntityPrimaryKeyValue = DesignerID
		...
		'Attaches SystemLoginID to Designer
		Set Conn = Server.CreateObject("ADODB.Connection")
		Conn.Open Connect
		SQLStmt = "UPDATE Castles_Designers SET "
		SQLStmt = SQLStmt & "SystemLoginID ='" & CreatedSystemLoginID & "' "
		SQLStmt = SQLStmt & "WHERE DesignerID ='" & EntityPrimaryKeyValue & "'"
		Set SQLAction = Conn.Execute(SQLStmt)
		Set SQLAction = Nothing
		Conn.Close
		...
		For i = 1 to HeaderCount
			If Len(Request.Form("TopNavigationHeaderID" & Count)) <> 0 Then
				...
				'Tracks Insertion of Designer Header Access Rights
				EntityID = 16
				EntityModificationTypeID = 1
				EntityModificationLogger SystemLoginID,EntityID,CreatedSystemLoginID,EntityModificationTypeID,IPAddress 
			End If
		...
		For i = 1 to SubHeaderCount
			If Len(Request.Form("TopNavigationSubHeaderID" & Count)) <> 0 Then
				...
				'Tracks Insertion of Designer SubHeader Access Rights
				EntityID = 16
				EntityModificationTypeID = 1
				EntityModificationLogger SystemLoginID,EntityID,CreatedSystemLoginID,EntityModificationTypeID,IPAddress 
			End If
		...
		If DuplicateFound = "N" Then
			DCDataDriverExecutedRedirectURL = "searchdesigners.asp?SearchColumn=" & Server.URLEncode(SearchColumn) & "&SearchValue=" & Server.URLEncode(SearchValue) & "&Page=" & Page & "&TotalRecords=" & TotalRecords
		Else
			DCDataDriverExecutedRedirectURL = "editdesigner.asp?SearchColumn=" & Server.URLEncode(SearchColumn) & "&SearchValue=" & Server.URLEncode(SearchValue) & "&Page=" & Page & "&TotalRecords=" & TotalRecords & "&DesignerID=" & DesignerID & "&DuplicateFound=" & DuplicateFound & "&OldUserName=" & Server.URLEncode(UserName) & "&AttemptUserName=" & Server.URLEncode(AttemptUserName)
		End If
	...
	Case "SQLUpdate"
		...
		'Tracks Deletion of Designer Access Rights
		EntityID = 16
	...
	Case "SQLMultiDelete"
		...
		'Deletes Designer SystemLogin Information		
