...
PageTopNavigationSubHeaderID = 41
EntityID = 11
%>
...
TranslateCount = TranslateCount + 1
WordPhrase_Subscriber = TranslationResultsArray(Field_TranslatedWordPhrase,TranslateCount)
TranslateCount = TranslateCount + 1
...
function PopNotes(EntityID,EntityPrimaryKeyValue,PersonalName){
	EntityName = "<%=WordPhrase_Subscriber%>"
        ...
        <tr> 
          <td> 
            <table width="750" border="0" cellspacing="0" cellpadding="3">
              ...
              <form name="SearchRecords" method="post" onSubmit="return Search();" action="searchsubscriptions.asp">
                <tr> 
...
%>
        ...
        <form name="DeleteRecords" method="post" onSubmit="return DeleteCheckedRecords();" action="drivesubscription.asp?DCDataDriverType=SQLMultiDelete&SearchColumn=<%=SearchColumn%>&SearchValue=<%=SearchValue%>&Page=<%=Page%>&TotalRecords=<%=TotalRecords%>">
          <tr> 
            <td> 
              <table width="100%" border="0" cellspacing="0" cellpadding="0" height="20">
                ...
                <tr height="20"> 
                  ...
                  <td bgcolor="<%=TitleBar%>" width="15%" class="CastlesTextHeader"><b>Name</b></td>
                  <td bgcolor="<%=TitleBar%>" width="1%">&nbsp;</td>
                  <td bgcolor="<%=TitleBar%>" width="20%" class="CastlesTextHeader"><b>Address</b></td>
                  <td bgcolor="<%=TitleBar%>" width="1%">&nbsp;</td>
...
if Len(SearchColumn) <> 0 then
	if Not IsNull(SearchResultsReturned) then
		...
		Field_ID = 0
		Field_SubscriptionID = 1
		Field_FirstName = 2
		Field_LastName = 3
		Field_BillingAddressLine1 = 4
		Field_BillingAddressLine2 = 5
		Field_BillingCity = 6
		Field_BillingStateProvinceAbv = 7
		Field_BillingZipPostalCode = 8
		Field_BillingCountryName = 9
		Field_TelNumber = 10
		Field_EmailAddress = 11
		Field_Active = 12
		Field_MoreRecords = 13

		For SearchResultsArrayRowCounter = 0 to SearchResultsArrayNumRows
			SubscriptionID = SearchResultsReturned(Field_SubscriptionID,SearchResultsArrayRowCounter)
			...
			BillingAddressLine1 = SearchResultsReturned(Field_BillingAddressLine1,SearchResultsArrayRowCounter)
			BillingAddressLine2 = SearchResultsReturned(Field_BillingAddressLine2,SearchResultsArrayRowCounter)
			BillingCity = SearchResultsReturned(Field_BillingCity,SearchResultsArrayRowCounter)
			BillingStateProvinceAbv = SearchResultsReturned(Field_BillingStateProvinceAbv,SearchResultsArrayRowCounter)
			BillingZipPostalCode = SearchResultsReturned(Field_BillingZipPostalCode,SearchResultsArrayRowCounter)
			BillingCountryName = SearchResultsReturned(Field_BillingCountryName,SearchResultsArrayRowCounter)

			Address = BillingAddressLine1 & "<br>"
			If Len(BillingAddressLine2) <> 0 Then
				Address = Address & BillingAddressLine2 & "<br>"
			End If
			Address = Address & BillingCity & ",&nbsp;" & BillingStateProvinceAbv & "&nbsp;" & BillingZipPostalCode & "<br>"
			Address = Address & BillingCountryName & "<BR>"
...
%>
                ...
                <tr bgcolor="<%=BgColor%>"> 
                  <td width="1%">&nbsp;</td>
                  <td width="15%" class="CastlesTextBody"><%=LastName%>,&nbsp;<%=FirstName%></td>
                  <td width="1%" align="center">&nbsp;</td>
                  <td width="20%" class="CastlesTextBody"><%=Address%></td>
                  <td width="1%" align="center">&nbsp;</td>
...
%>
                    <a href="viewsubscription.asp?SubscriptionID=<%=SubscriptionID%>&SearchColumn=<%=Server.URLEncode(SearchColumn)%>&SearchValue=<%=Server.URLEncode(SearchValue)%>&Page=<%=Page%>&TotalRecords=<%=TotalRecords%>" class="normal"><b><%=WordPhrase_View%></b></a>&nbsp;&nbsp;&nbsp;&nbsp; 
                    <%
End If
If inStr(EntityModificationAccessString,"(2)" ) <> 0 Then
%>
                    <a href="editsubscription.asp?SubscriptionID=<%=SubscriptionID%>&SearchColumn=<%=Server.URLEncode(SearchColumn)%>&SearchValue=<%=Server.URLEncode(SearchValue)%>&Page=<%=Page%>&TotalRecords=<%=TotalRecords%>" class="normal"><b><%=WordPhrase_Edit%></b></a>&nbsp;&nbsp;&nbsp;&nbsp; 
                    <%
End If
If inStr(EntityModificationAccessString,"(6)" ) <> 0 Then
FullName = FirstName & " " & LastName
%>
                    <a href="javascript:onClick=PopNotes('1','<%=SubscriptionID%>','<%=Server.URLEncode(FullName)%>');" class="normal"><b><%=WordPhrase_Notes%></b></a> 
                    <%
End If
%>
                  </td>
...
If inStr(EntityModificationAccessString,"(3)" ) <> 0 Then
%>
                    <input type="checkbox" name="DeleteRecordID<%=DeleteCount%>" value="<%=SubscriptionID%>">
                    <%
End If
%>
                    &nbsp;&nbsp;</td>
...
If Len(SearchColumn) <> 0 Then
	PageName = "searchSubscriptions.asp"
