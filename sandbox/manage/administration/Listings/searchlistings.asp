...
PageTopNavigationSubHeaderID = 29
EntityID = 10
%>
...
'SearchedAreas = findValue("Areas")
SearchedStates = findValue("SearchedStates")
'response.write SearchedAreas & "<br>"
...
'response.write "MoreRecords: " & MoreRecords & "<br>"
...
if (SearchListingID<>"")then
	set Command1 = Server.CreateObject("ADODB.Command")
		with command1	
			.ActiveConnection = connect
			.CommandText = "Castles_System_Listings_SearchbyID"
			.Parameters.Append .CreateParameter("@RETURN_VALUE", 3, 4)
...
if ((SearchListingID="")AND(SearchedAddress<>""))then
	set Command1 = Server.CreateObject("ADODB.Command")
		with command1	
			.ActiveConnection = connect
			.CommandText = "Castles_System_Listings_SearchbyAddress"
			.Parameters.Append .CreateParameter("@RETURN_VALUE", 3, 4)
			.Parameters.Append .CreateParameter("@Page", 200, 1,200,Page)
			.Parameters.Append .CreateParameter("@RecsPerPage", 200, 1,200,RecsPerPage)
...
if ((SearchListingID="")and(SearchedAddress="")and(SearchedStates<>"")and(SearchedSizes<>"")) then
	...
	if (NOT StatusNoFilter AND NOT SizesNoFilter AND NOT StatesNoFilter) then	
		SQLStmt = SQLStmt & " Castles_Listings.Deleted <> 'Y' AND Castles_Listings.Active = '" & SearchedApartmentStatus & "' AND Castles_Listings.StateProvinceID ="	
		for each z in SizesArray 
			for each x in StatesArray
				if counter = 1 then
					SQLStmt = SQLStmt & Trim(x) 
				else
					SQLStmt = SQLStmt & " OR Castles_Listings.Deleted <> 'Y' AND Castles_Listings.StateProvinceID = " & Trim(x) & " AND Castles_Listings.Active = '" & SearchedApartmentStatus	& "'"
				end if				
	...
	if (NOT StatusNoFilter AND NOT SizesNoFilter AND StatesNoFilter) then	
		SQLStmt = SQLStmt & " Castles_Listings.Deleted <> 'Y' AND Castles_Listings.Active = '" & SearchedApartmentStatus & "' AND Castles_Listings.PropertyTypeID ="	
		for each z in SizesArray 
			if counter = 1 then
				SQLStmt = SQLStmt & Trim(z) 
			else
				SQLStmt = SQLStmt & " OR Castles_Listings.Deleted <> 'Y' AND Castles_Listings.Active = '" & SearchedApartmentStatus & "'"	
			end if				
	...
	if (StatusNoFilter AND NOT SizesNoFilter AND StatesNoFilter) then	
		SQLStmt = SQLStmt & " Castles_Listings.Deleted <> 'Y' AND Castles_Listings.PropertyTypeID ="	
		for each z in SizesArray 
			if counter = 1 then
				SQLStmt = SQLStmt &  Trim(z)
			else
				SQLStmt = SQLStmt & " OR Castles_Listings.Deleted <> 'Y' AND Castles_Listings.PropertyTypeID = " & Trim(z) 	
			end if				
			SQLStmt = SQLStmt & " AND Castles_Listings.ListPrice >= " & PriceFrom & " AND Castles_Listings.ListPrice <= " & PriceTo
			counter = counter + 1
		Next
	...
	if (NOT StatusNoFilter AND SizesNoFilter AND StatesNoFilter) then	
		SQLStmt = SQLStmt & " Castles_Listings.Deleted <> 'Y' AND Castles_Listings.Active = '" & SearchedApartmentStatus & "'"						
		SQLStmt = SQLStmt & " AND Castles_Listings.ListPrice >= " & PriceFrom & " AND Castles_Listings.ListPrice <= " & PriceTo
	end if
	
	if (StatusNoFilter AND SizesNoFilter AND StatesNoFilter) then	
		SQLStmt = SQLStmt & " Castles_Listings.Deleted <> 'Y'"					
		SQLStmt = SQLStmt & " AND Castles_Listings.ListPrice >= " & PriceFrom & " AND Castles_Listings.ListPrice <= " & PriceTo
	end if
	...
	if (NOT StatusNoFilter AND SizesNoFilter AND NOT StatesNoFilter) then	
		...
		for each x in StatesArray
			if counter = 1 then
				SQLStmt = SQLStmt & Trim(x) & " AND Castles_Listings.Active = '" & SearchedApartmentStatus & "'"
			else
				SQLStmt = SQLStmt & " OR Castles_Listings.Deleted <> 'Y' AND Castles_Listings.StateProvinceID = " & Trim(x) & " AND Castles_Listings.Active = '" & SearchedApartmentStatus 	& "'"
			end if				
			SQLStmt = SQLStmt & " AND Castles_Listings.ListPrice >= " & PriceFrom & " AND Castles_Listings.ListPrice <= " & PriceTo
	...
	if (StatusNoFilter AND NOT SizesNoFilter AND NOT StatesNoFilter) then
		SQLStmt = SQLStmt & " Castles_Listings.Deleted <> 'Y' AND Castles_Listings.StateProvinceID = "	
		for each z in SizesArray 
			for each x in StatesArray
				if counter = 1 then
					SQLStmt = SQLStmt & Trim(x) 
				else
					SQLStmt = SQLStmt & " OR Castles_Listings.Deleted <> 'Y' AND Castles_Listings.StateProvinceID = " & Trim(x)	
				end if				
				SQLStmt = SQLStmt & " AND Castles_Listings.PropertyTypeID = " & Trim(z) & " AND Castles_Listings.ListPrice >= " & PriceFrom & " AND Castles_Listings.ListPrice <= " & PriceTo
				counter = counter + 1
			Next
	...
	set Command1 = Server.CreateObject("ADODB.Command")
		with command1	
			.ActiveConnection = connect
			.CommandText = "Castles_System_Listings_Manage_Search"
			.Parameters.Append .CreateParameter("@RETURN_VALUE", 3, 4)
			.Parameters.Append .CreateParameter("@Page", 200, 1,200,Page)
			.Parameters.Append .CreateParameter("@RecsPerPage", 200, 1,200,RecsPerPage)
...
set Command1 = Server.CreateObject("ADODB.Command")
	with command1	
		.ActiveConnection = connect
		.CommandText = "Castles_System_PropertyTypes_Admin_DynamicList"
		.Parameters.Append .CreateParameter("@RETURN_VALUE", 3, 4)
		.CommandType = 4
		.CommandTimeout = 0
		.Prepared = true
...
%>		   
            <table width="750" border="0" cellspacing="0" cellpadding="3">
              ...
              <form name="SearchRecords" method="post" onSubmit="return Search();" action="searchlistings.asp">
                <tr> 
                  ...
                  <td class="CastlesTextBody" width="385">[ 3 ] Listing Status:</td>
                </tr>
				<tr> 
                  ...
                  <td class="CastlesTextBody" width="385"> 
                    <select name="SearchedApartmentStatus" class="CastlesTextBody">
                      <%
if SearchedApartmentStatus = "ALL" then
%>
...
'While (NOT ListingStatus.EOF)
	'if cStr(ListingStatus.Fields.Item("ListingStatusID").Value) = SearchedApartmentStatus then%>
                      <!--<option value="<%'=Trim(ListingStatus.Fields.Item("ListingStatusID").Value)%>" selected><%'=(ListingStatus.Fields.Item("ListingStatusName").Value)%></option>-->
                      <%'else%>
                      <!--<option value="<%'=Trim(ListingStatus.Fields.Item("ListingStatusID").Value)%>"><%'=(ListingStatus.Fields.Item("ListingStatusName").Value)%></option>-->
                      <%'end if
...
%>
<%
if SearchedApartmentStatus = "Y" then
%>
					<option value="Y" selected>Currently Listed</option>
					<%
else
%>
                      <option value="Y">Currently Listed</option>
                      <%
end if
%>
<%
if SearchedApartmentStatus = "N" then
%>
					<option value="N" selected>Currently NotListed</option>
					<%
else
%>
                      <option value="N">Currently NotListed</option>
                      <%
end if
%>
                    </select>
...
%>						
        ...
        <form name="DeleteRecords" method="post" onSubmit="return DeleteCheckedRecords();" action="drivelistings.asp?DCDataDriverType=SQLMultiDelete&SearchListingID=<%=SearchListingID%>&SearchedAddress=<%=SearchedAddress%>&SearchedSizes=<%=SearchedSizes%>&SearchedStates=<%=SearchedStates%>&SearchedApartmentStatus=<%=SearchedApartmentStatus%>&PriceFrom=<%=PriceFrom%>&PriceTo=<%=PriceTo%>&Page=<%=Page%>&TotalRecords=<%=TotalRecords%>">
		  ...
		  <td bgcolor="<%=TitleBar%>" width="1%">&nbsp;</td>
                  <td bgcolor="<%=TitleBar%>" width="5%" class="CastlesTextHeader"><b>ListingID</b></td>
		  <td bgcolor="<%=TitleBar%>" width="1%">&nbsp;</td>
                  ...
                  <td bgcolor="<%=TitleBar%>" width="1%">&nbsp;</td>
                  <td bgcolor="<%=TitleBar%>" width="8%" class="CastlesTextHeader" align="center">&nbsp;&nbsp; 
                    <%
...
%>
                <tr> 
                  <td colspan="19" height="1"><img src="/manage/images/clear10pixel.gif" width="1" height="1"></td>
                </tr>
                <tr bgcolor="<%=ThinLine%>"> 
                  <td colspan="19" height="1"><img src="/manage/images/clear10pixel.gif" width="1" height="1"></td>
                </tr>
                ...
                <tr bgcolor="<%=BgColor%>"> 
		  <td width="1%">&nbsp;</td>
                  <td width="5%" class="CastlesTextBody"><%=ListingID%></td>
                  <td width="1%">&nbsp;</td>
...
%>
                    <a href="viewlisting.asp?ListingID=<%=ListingID%>&SearchListingID=<%=SearchListingID%>&SearchedAddress=<%=Server.URLEncode(SearchedAddress)%>&SearchedSizes=<%=Server.URLEncode(SearchedSizes)%>&SearchedStates=<%=Server.URLEncode(SearchedStates)%>&SearchedApartmentStatus=<%=Server.URLEncode(SearchedApartmentStatus)%>&PriceFrom=<%=Server.URLEncode(PriceFrom)%>&PriceTo=<%=Server.URLEncode(PriceTo)%>&Page=<%=Page%>&TotalRecords=<%=TotalRecords%>" class="normal"><b><%=WordPhrase_View%></b></a>&nbsp;&nbsp;&nbsp;&nbsp; 
                    <%
End If
If inStr(EntityModificationAccessString,"(2)" ) <> 0 Then
%>
                    <a href="editlisting.asp?ListingID=<%=ListingID%>&SearchListingID=<%=SearchListingID%>&SearchedAddress=<%=Server.URLEncode(SearchedAddress)%>&SearchedSizes=<%=Server.URLEncode(SearchedSizes)%>&SearchedStates=<%=Server.URLEncode(SearchedStates)%>&SearchedApartmentStatus=<%=Server.URLEncode(SearchedApartmentStatus)%>&PriceFrom=<%=Server.URLEncode(PriceFrom)%>&PriceTo=<%=Server.URLEncode(PriceTo)%>&Page=<%=Page%>&TotalRecords=<%=TotalRecords%>" class="normal"><b><%=WordPhrase_Edit%></b></a>&nbsp;&nbsp;&nbsp;&nbsp; 
                    <%
End If
...
%>
                  ...
                  <td width="1%" align="center">&nbsp;</td>
                  <td width="8%" class="CastlesTextBody" align="center">&nbsp;&nbsp; 
                    <%
If inStr(EntityModificationAccessString,"(3)" ) <> 0 Then
%>
                    <input type="checkbox" name="DeleteRecordID<%=DeleteCount%>" value="<%=ListingID%>">
                    <%
End If
%>
                    &nbsp;&nbsp;</td>
...
%>
                ...
                <tr bgcolor="<%=BgColor%>"> 
                  <td width="1%">&nbsp;</td>
                  <td colspan="19" class="CastlesTextBody"><%=WordPhrase_NoResultsFound%>...</td>
                </tr>
                <%
			Count = Count+1
		Next
...
Function DCSystemSimpleSearchPagination(Page,WordPhrase_JumpToPage,PageName,MoreRecords,TotalRecords,RecsPerPage,SearchListingID,SearchedAddress,SearchedSizes,SearchedStates,SearchedApartmentStatus,PriceFrom,PriceTo)
	...
	For i = 1 to Paginations 
		If cStr(PageCount) <> cStr(Page) Then
			DCSystemSimpleSearchPagination = DCSystemSimpleSearchPagination &  "<a href=" & DQ & PageName & "?SearchListingID=" & Server.URLEncode(SearchListingID) & "&SearchedAddress=" & Server.URLEncode(SearchedAddress) & "&SearchedSizes=" & Server.URLEncode(SearchedSizes) & "&SearchedStates=" & Server.URLEncode(SearchedStates) &  "&SearchedApartmentStatus=" & Server.URLEncode(SearchedApartmentStatus) & "&PriceFrom=" & Server.URLEncode(PriceFrom) & "&PriceTo=" & Server.URLEncode(PriceTo) & "&Page=" & PageCount & "&TotalRecords=" & TotalRecords & DQ &" class=""orange""><b>" & PageCount & "</b></a>|"
		Else
