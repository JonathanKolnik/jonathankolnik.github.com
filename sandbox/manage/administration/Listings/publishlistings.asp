...
PageTopNavigationSubHeaderID = 39
EntityID = 10
%>
...
if ((SearchListingID="")and(SearchedAddress="")and(SearchedStates<>"")and(SearchedSizes<>"")) then
	...
	if (NOT StatusNoFilter AND NOT SizesNoFilter AND NOT StatesNoFilter) then	
		SQLStmt = SQLStmt & " Castles_Listings.Deleted <> 'Y' AND Castles_Listings.ListingPublishStatusID = 2 AND Castles_Listings.StatusID = '" & SearchedApartmentStatus & "' AND Castles_Listings.StateProvinceID ="	
		for each z in SizesArray 
			for each x in StatesArray
				if counter = 1 then
					SQLStmt = SQLStmt & Trim(x) 
				else
					SQLStmt = SQLStmt & " OR Castles_Listings.Deleted <> 'Y' AND Castles_Listings.ListingPublishStatusID = 2 AND Castles_Listings.StateProvinceID = " & Trim(x) & " AND Castles_Listings.StatusID = " & SearchedApartmentStatus	
				end if				
	...
	if (NOT StatusNoFilter AND NOT SizesNoFilter AND StatesNoFilter) then	
		SQLStmt = SQLStmt & " Castles_Listings.Deleted <> 'Y' AND Castles_Listings.ListingPublishStatusID = 2 AND Castles_Listings.StatusID = '" & SearchedApartmentStatus & "' AND Castles_Listings.PropertyTypeID ="	
		for each z in SizesArray 
			if counter = 1 then
				SQLStmt = SQLStmt & Trim(z) 
			else
				SQLStmt = SQLStmt & " OR Castles_Listings.Deleted <> 'Y' AND Castles_Listings.ListingPublishStatusID = 2 AND Castles_Listings.StatusID = " & SearchedApartmentStatus 	
			end if				
	...
	if (StatusNoFilter AND NOT SizesNoFilter AND StatesNoFilter) then	
		SQLStmt = SQLStmt & " Castles_Listings.Deleted <> 'Y' AND Castles_Listings.ListingPublishStatusID = 2 AND Castles_Listings.PropertyTypeID ="	
		for each z in SizesArray 
			if counter = 1 then
				SQLStmt = SQLStmt &  Trim(z)
			else
				SQLStmt = SQLStmt & " OR Castles_Listings.Deleted <> 'Y' AND Castles_Listings.ListingPublishStatusID = 2 AND Castles_Listings.PropertyTypeID = " & Trim(z) 	
			end if				
			SQLStmt = SQLStmt & " AND Castles_Listings.ListPrice >= " & PriceFrom & " AND Castles_Listings.ListPrice <= " & PriceTo
			counter = counter + 1
		Next
	...
	if (NOT StatusNoFilter AND SizesNoFilter AND StatesNoFilter) then	
		SQLStmt = SQLStmt & " Castles_Listings.Deleted <> 'Y' AND Castles_Listings.ListingPublishStatusID = 2 AND Castles_Listings.StatusID = " & SearchedApartmentStatus 						
		SQLStmt = SQLStmt & " AND Castles_Listings.ListPrice >= " & PriceFrom & " AND Castles_Listings.ListPrice <= " & PriceTo
	end if
	
	if (StatusNoFilter AND SizesNoFilter AND StatesNoFilter) then	
		SQLStmt = SQLStmt & " Castles_Listings.Deleted <> 'Y' AND Castles_Listings.ListingPublishStatusID = 2"					
		SQLStmt = SQLStmt & " AND Castles_Listings.ListPrice >= " & PriceFrom & " AND Castles_Listings.ListPrice <= " & PriceTo
	end if
	...
	if (StatusNoFilter AND NOT SizesNoFilter AND NOT StatesNoFilter) then
		SQLStmt = SQLStmt & " Castles_Listings.Deleted <> 'Y' AND Castles_Listings.ListingPublishStatusID = 2 AND Castles_Listings.StateProvinceID = "	
		for each z in SizesArray 
			for each x in StatesArray
				if counter = 1 then
					SQLStmt = SQLStmt & Trim(x) 
				else
					SQLStmt = SQLStmt & " OR Castles_Listings.Deleted <> 'Y' AND Castles_Listings.ListingPublishStatusID = 2 AND Castles_Listings.StateProvinceID = " & Trim(x)	
				end if				
				SQLStmt = SQLStmt & " AND Castles_Listings.PropertyTypeID = " & Trim(z) & " AND Castles_Listings.ListPrice >= " & PriceFrom & " AND Castles_Listings.ListPrice <= " & PriceTo
				counter = counter + 1
			Next
...
%>		   
            <table width="750" border="0" cellspacing="0" cellpadding="3">
              ...
              <form name="SearchRecords" method="post" onSubmit="return Search();" action="publishlistings.asp">
                <tr> 
...
%>
        ...
        <form name="ApproveRecords" method="post" action="driveListingsPublishApprove.asp?SearchListingID=<%=SearchListingID%>&SearchedAddress=<%=SearchedAddress%>&SearchedSizes=<%=SearchedSizes%>&SearchedAreas=<%=SearchedAreas%>&SearchedStatus=<%=SearchedApartmentStatus%>&PriceFrom=<%=PriceFrom%>&PriceTo=<%=PriceTo%>&Page=<%=Page%>&TotalRecords=<%=TotalRecords%>">
          <tr> 
            <td> 
              <table width="100%" border="0" cellspacing="0" cellpadding="0" height="20">
                ...
                <tr height="20"> 
                  ...
                  <td bgcolor="<%=TitleBar%>" width="13%" class="CastlesTextHeader" align="center">&nbsp;&nbsp; 
                    <%
If inStr(EntityModificationAccessString,"(7)" ) <> 0 Then
%>
                    <b>Approve</b> 
                    <%
End If
...
%>
                    <a href="viewlisting.asp?ListingID=<%=ListingID%>&SearchListingID=<%=SearchListingID%>&SearchedAddress=<%=Server.URLEncode(SearchedAddress)%>&SearchedSizes=<%=Server.URLEncode(SearchedSizes)%>&SearchedAreas=<%=Server.URLEncode(SearchedAreas)%>&SearchedStatus=<%=Server.URLEncode(SearchedApartmentStatus)%>&PriceFrom=<%=Server.URLEncode(PriceFrom)%>&PriceTo=<%=Server.URLEncode(PriceTo)%>&Page=<%=Page%>&TotalRecords=<%=TotalRecords%>" class="normal"><b><%=WordPhrase_View%></b></a>&nbsp;&nbsp;&nbsp;&nbsp; 
                    <%
End If
...
If inStr(EntityModificationAccessString,"(7)" ) <> 0 Then
%>
                    <input type="checkbox" name="ApproveRecordID<%=ApproveCount%>" value="<%=ListingID%>">
                    <%
End If
%>
                    &nbsp;&nbsp;</td>
...
%>
                ...
                <input type="hidden" name="NumberOfRecordsToApprove" value="<%=ApproveCount%>">
              </table>
            </td>
          </tr>
          <tr> 
...
Function DCSystemSimpleSearchPagination(Page,WordPhrase_JumpToPage,PageName,MoreRecords,TotalRecords,RecsPerPage,SearchListingID,SearchedAddress,SearchedSizes,SearchedAreas,SearchedApartmentStatus,PriceFrom,PriceTo)
	If Page <> 1 Then
		TotalRecords = Request.QueryString("TotalRecords")
	End If
...
'If Len(SQLStmt) <> 0 Then
	...
	PaginationCode = DCSystemSimpleSearchPagination(Page,WordPhrase_JumpToPage,PageName,MoreRecords,TotalRecords,RecsPerPage,SearchListingID,SearchedAddress,SearchedSizes,SearchedAreas,SearchedApartmentStatus,PriceFrom,PriceTo)
...
If inStr(EntityModificationAccessString,"(7)" ) <> 0 Then
%>
                    <input type="submit" name="Submit" value="Approve" class="CastlesTextBlack">
                    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 
                    <%
End If
%>
