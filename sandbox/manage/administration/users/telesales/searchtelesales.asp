...
PageTopNavigationSubHeaderID = 11
EntityID = 4
%>
...
WordPhrasesOnPage = "Actions(|)Active(|)Delete(|)Direct Line(|)Edit(|)Email Address(|)Jump To Page(|)Mobile Phone(|)Name(|)No Results Found(|)Notes(|)Search(|)Search By(|)Search Field(|)Search Value(|)TeleSales(|)View"
...
TranslateCount = TranslateCount + 1
WordPhrase_TeleSales = TranslationResultsArray(Field_TranslatedWordPhrase,TranslateCount)
TranslateCount = TranslateCount + 1
...
function PopNotes(EntityID,EntityPrimaryKeyValue,PersonalName){
	EntityName = "<%=WordPhrase_Telesales%>"
        ...
        <tr> 
          <td> 
            <table width="750" border="0" cellspacing="0" cellpadding="3">
              ...
              <form name="SearchRecords" method="post" onSubmit="return Search();" action="searchtelesales.asp">
                <tr> 
...
%>
        ...
        <form name="DeleteRecords" method="post" onSubmit="return DeleteCheckedRecords();" action="drivetelesales.asp?DCDataDriverType=SQLMultiDelete&SearchColumn=<%=SearchColumn%>&SearchValue=<%=SearchValue%>&Page=<%=Page%>&TotalRecords=<%=TotalRecords%>">
          <tr> 
          <td> 
...
if Len(SearchColumn) <> 0 then
	if Not IsNull(SearchResultsReturned) then
		...
		For SearchResultsArrayRowCounter = 0 to SearchResultsArrayNumRows
			TelesalesID = SearchResultsReturned(Field_TelesalesID,SearchResultsArrayRowCounter)
...
%>
                    <a href="viewtelesales.asp?TelesalesID=<%=TelesalesID%>&SearchColumn=<%=Server.URLEncode(SearchColumn)%>&SearchValue=<%=Server.URLEncode(SearchValue)%>&Page=<%=Page%>&TotalRecords=<%=TotalRecords%>" class="normal"><b><%=WordPhrase_View%></b></a>&nbsp;&nbsp;&nbsp;&nbsp; 
                    <%
End If
If inStr(EntityModificationAccessString,"(2)" ) <> 0 Then
%>
                    <a href="edittelesales.asp?TelesalesID=<%=TelesalesID%>&SearchColumn=<%=Server.URLEncode(SearchColumn)%>&SearchValue=<%=Server.URLEncode(SearchValue)%>&Page=<%=Page%>&TotalRecords=<%=TotalRecords%>" class="normal"><b><%=WordPhrase_Edit%></b></a>&nbsp;&nbsp;&nbsp;&nbsp; 
                    <%
End If
If inStr(EntityModificationAccessString,"(6)" ) <> 0 Then
FullName = FirstName & " " & LastName
%>
                    <a href="javascript:onClick=PopNotes('1','<%=TelesalesID%>','<%=Server.URLEncode(FullName)%>');" class="normal"><b><%=WordPhrase_Notes%></b></a> 
                    <%
End If
%>
                  </td>
...
If inStr(EntityModificationAccessString,"(3)" ) <> 0 Then
%>
                    <input type="checkbox" name="DeleteRecordID<%=DeleteCount%>" value="<%=TelesalesID%>">
<%
End If
%>
                    &nbsp;&nbsp;</td>
