...
Select Case DCDataDriverType
	Case "SQLInsert"
		...
		DBObjectDestination = "Castles_Administrators"
		FormType = "Request.Form"
		FileServerDestination = ""
		FileFate = ""
		EmailDestination = ""
		DataUniqueKey = "AdministratorID"
		DataParentNode = ""
		DataExceptions = "<!AdministratorID!><!EntityLanguageID!><!EntitySystemLoginID!><!HeaderCount!><!SubHeaderCount!><!NickName!><!UserName!><!Password!><!ShowHelpContent!><!CreateNotes!><!ViewNotes!><!DeleteNotes!><!Submit!>" & DataExceptionsString
		DataCookies = ""
		DataSessions = ""
		DataExtraFields = ""
		Cnekt = Connect
		
		AdministratorID = DCDataDriver(DCDataDriverType,DBObjectDestination,FileServerDestination,FileFate,EmailDestination,DataUniqueKey,DataParentNode,DataCookies,DataSessions,DataExtraFields,DataExceptions,Cnekt,EntityID,FormType)
		
		'Tracks Modification of Administrator Account
		EntityID = 1
		EntityModificationTypeID = 2
		EntityModificationLogger SystemLoginID,EntityID,AdministratorID,EntityModificationTypeID,IPAddress 

		'Creates System Login Username & Password
		SystemLoginTypeID = 1
		EntityID = 1
		Deleted = "N"
		EntityPrimaryKeyValue = AdministratorID
		...
		'Attaches SystemLoginID to Administrator
		Set Conn = Server.CreateObject("ADODB.Connection")
		Conn.Open Connect
		SQLStmt = "UPDATE Castles_Administrators SET "
		SQLStmt = SQLStmt & "SystemLoginID ='" & CreatedSystemLoginID & "' "
		SQLStmt = SQLStmt & "WHERE AdministratorID ='" & EntityPrimaryKeyValue & "'"
		Set SQLAction = Conn.Execute(SQLStmt)
		Set SQLAction = Nothing
		Conn.Close
		...
		For i = 1 to HeaderCount
			If Len(Request.Form("TopNavigationHeaderID" & Count)) <> 0 Then
				...
				'Tracks Insertion of Administrator Header Access Rights
				EntityID = 16
				EntityModificationTypeID = 1
				EntityModificationLogger SystemLoginID,EntityID,CreatedSystemLoginID,EntityModificationTypeID,IPAddress 
			End If
		...
		For i = 1 to SubHeaderCount
			If Len(Request.Form("TopNavigationSubHeaderID" & Count)) <> 0 Then
				...
				'Tracks Insertion of Administrator SubHeader Access Rights
				EntityID = 16
				EntityModificationTypeID = 1
				EntityModificationLogger SystemLoginID,EntityID,CreatedSystemLoginID,EntityModificationTypeID,IPAddress 
			End If
		...
		If DuplicateFound = "N" Then
			DCDataDriverExecutedRedirectURL = "searchadministrators.asp?SearchColumn=" & Server.URLEncode(SearchColumn) & "&SearchValue=" & Server.URLEncode(SearchValue) & "&Page=" & Page & "&TotalRecords=" & TotalRecords
		Else
			DCDataDriverExecutedRedirectURL = "editadministrator.asp?SearchColumn=" & Server.URLEncode(SearchColumn) & "&SearchValue=" & Server.URLEncode(SearchValue) & "&Page=" & Page & "&TotalRecords=" & TotalRecords & "&AdministratorID=" & AdministratorID & "&DuplicateFound=" & DuplicateFound & "&OldUserName=" & Server.URLEncode(UserName) & "&AttemptUserName=" & Server.URLEncode(AttemptUserName)
		End If
	...
	Case "SQLUpdate"
		...
		'Tracks Deletion of Administrator Access Rights
		EntityID = 16
	...
	Case "SQLMultiDelete"
		...
		'Deletes Administrator SystemLogin Information		
